...
// Dispatches on `tag` and produces a !util.varlen32-typed Value holding the
// canonical lexical form of the variant (tag, ref) -- shared by
// ToStringOpLowering and StrCastOpLowering so the runtime-dispatch cascade
// (Unspecified guard / numeric-family / RDFNode / string-passthrough /
// blob-literal-fallback) is written exactly once.
//
// The Unspecified guard must come first: an Unspecified-tagged variant (e.g.
// produced by variant.unspecified_if for an unmatched OPTIONAL binding)
// carries a genuinely null `ref` (util.InvalidRefOp lowers to a null
// pointer), so it must never reach toStringBlobLiteral(ref)'s ref->payload
// dereference below.
...
// variant.cast's supported target set: Boolean plus the fixed-width numeric
// family -- exactly the tags variant.create_scalar's lowering can itself
// produce (see tagForScalarType above). Arbitrary-precision xsd:integer/
// xsd:decimal targets are folded onto Long/Double by the SPARQL frontend
// before this op is ever emitted (see CastOp's description in VariantOps.td).
bool isSupportedCastTarget(xsd::Type t) {
...
class CastOpLowering : public OpConversionPattern<variant::CastOp> {
    ...
    LogicalResult matchAndRewrite(variant::CastOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override {
        ...
        // typeId must resolve to a compile-time constant (see CastOp's
        // description in VariantOps.td) -- no dynamic-target dispatch.
        llvm::APInt constVal;
        ...
        auto result = rewriter.create<scf::IfOp>(
            loc, sameTag,
            [&](OpBuilder& b, Location l) {
                // Identity: same tag => no-op passthrough of the (already
                // type-converted) input tuple.
