    ...
    func.func @main() {
    	%subop_result = subop.execution_group (){
            ...
            %vx = subop.nested_map %g_scan [@edges::@set] (%arg0, %arg1){
                %node_stream = gsubop.scan_edge_set %arg1 : !gsubop.edge_set<[ex_it : !gsubop.graph_set_iterator<["all"]>]> @edges::@ref({type = !gsubop.edge_ref<[edge_id : i32],[from : !gsubop.node_ref<[node_id1 : i32],[incoming1 : !gsubop.edge_set<[incoming_it1 : !gsubop.graph_set_iterator<["incoming"]>]>],[outgoing1 : !gsubop.edge_set<[outgoing_it1 : !gsubop.graph_set_iterator<["outgoing"]>]>],[property1 : i64]>],[to : !gsubop.node_ref<[node_id2 : i32],[incoming2 : !gsubop.edge_set<[incoming_it2 : !gsubop.graph_set_iterator<["incoming"]>]>],[outgoing2 : !gsubop.edge_set<[outgoing_it2 : !gsubop.graph_set_iterator<["outgoing"]>]>],[property2 : i64]>],[edge_prop : i64]>})
                tuples.return %node_stream : !tuples.tuplestream
            }
            %result_edges = subop.gather %vx @edges::@ref {edge_id => @edges::@id({type = i32})}
            %result_nodes0 = subop.gather %result_edges @edges::@ref {from => @nodes::@from({type = !gsubop.node_ref<[node_id1 : i32],[incoming1 : !gsubop.edge_set<[incoming_it1 : !gsubop.graph_set_iterator<["incoming"]>]>],[outgoing1 : !gsubop.edge_set<[outgoing_it1 : !gsubop.graph_set_iterator<["outgoing"]>]>],[property1 : i64]>})}
