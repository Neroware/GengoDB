module {
    func.func @main() {
    	%subop_result = subop.execution_group (){
            ...
            %toNodes = subop.gather %ex @edges::@ref {to => @edges::@toRef({type = !gsubop.node_ref<[node_id2 : i32],[incoming2 : !gsubop.edge_set<[incoming_it2 : !gsubop.graph_set_iterator<["incoming"]>]>],[outgoing2 : !gsubop.edge_set<[outgoing_it2 : !gsubop.graph_set_iterator<["outgoing"]>]>],[property2 : i64]>})}
            %toNodes0 = subop.gather %toNodes @edges::@toRef {node_id2 => @nodes2::@id({type = i32})}

            %0 = subop.create !subop.result_table<[int32p0 : i32]>
            subop.materialize %toNodes0 {@nodes2::@id => int32p0}, %0 : !subop.result_table<[int32p0 : i32]>
