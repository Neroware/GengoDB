    ...
    func.func @main() {
    	%subop_result = subop.execution_group (){
            ...
            %props = subop.nested_map %px [@props::@set] (%arg0, %arg1){
                ...
                %prop_stream1 = gsubop.cast_property_ref %prop_stream0 @props::@refs -> @props::@refsStr({type = !gsubop.typed_property_ref<[property_str : !db.string]>})
                tuples.return %prop_stream1 : !tuples.tuplestream
            }

            %result_props = subop.gather %props @props::@refsStr {property_str => @props::@result({type = !db.string})}

            %0 = subop.create !subop.result_table<[int32p0 : i32, strp1 : !db.string]>
            subop.materialize %result_props {@nodes::@id => int32p0, @props::@result => strp1}, %0 : !subop.result_table<[int32p0 : i32, strp1 : !db.string]>
