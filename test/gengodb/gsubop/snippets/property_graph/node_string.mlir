    ...
    func.func @main() {
    	%subop_result = subop.execution_group (){
            ...
            %props = subop.nested_map %px [@props::@set] (%arg0, %arg1){
                ...
                tuples.return %prop_stream0 : !tuples.tuplestream
            }
            %props_str = gsubop.ref_to_str %props @nodes::@ref -> @props::@result({type = !db.string})

            %0 = subop.create !subop.result_table<[int32p0 : i32, strp1 : !db.string]>
            subop.materialize %props_str {@nodes::@id => int32p0, @props::@result => strp1}, %0 : !subop.result_table<[int32p0 : i32, strp1 : !db.string]>
