    ...
    func.func @main() {
        %res = subop.execution_group (){
            ...
            %filtered = relalg.selection %bgp (%arg0 : !tuples.tuple){
                ...
                %oneVar = variant.create_scalar %one : i32
                %sum = variant.arith add %ageVar, %oneVar
                %thirty = arith.constant 30 : i32
                %thirtyVar = variant.create_scalar %thirty : i32
                %pred = variant.cmp gt %sum, %thirtyVar -> !db.nullable<i1>
